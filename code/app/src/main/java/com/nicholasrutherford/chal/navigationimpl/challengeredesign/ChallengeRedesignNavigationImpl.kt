...
import com.nicholasrutherford.chal.fragments.challengeDetailsFragment
    ...
    override fun showChallengeDetails(activity: MainActivity, context: Context, isClicked: Boolean, fragmentManager: FragmentManager, id: Int, bottomNavigationView: BottomNavigationView) {
        if (isClicked) {
            ...
            fragmentManager.beginTransaction()
                .replace(id, challengeDetailsFragment(activity, context),
                    challengeDetailsFragment(activity, context)::javaClass.name)
                .commit()
        }
    }
